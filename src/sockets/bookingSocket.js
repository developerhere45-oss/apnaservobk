...
function serializeBooking(booking) {
  ...
  const publicBookingId = String(doc.bookingId || doc.publicId || doc.bookingCode || doc._id || "");
  ...
  return {
    ...
    additionalWork: doc.workCompletion?.additionalWork || "",
    partnerNotes: doc.workCompletion?.partnerNotes || "",
    workSubmittedBy: doc.workCompletion?.submittedBy || "",
    workSubmittedAt: doc.workCompletion?.submittedAt || null,
    workSubmittedAtMillis: millis(doc.workCompletion?.submittedAt),
    serviceWorkDetails: doc.serviceWorkDetails || {
      description: doc.workCompletion?.additionalWork || "",
      completedTasks: (doc.workCompletion?.completedTasks || []).map((name, index) => ({ taskId: `legacy_${index}`, name })),
      customWork: [],
      additionalNotes: doc.workCompletion?.partnerNotes || "",
      checklistVersion: 1,
      submittedBy: doc.workCompletion?.submittedBy || "",
      submittedAt: doc.workCompletion?.submittedAt || null
    },
    ...
    requestExpiredAt: doc.requestExpiredAt || null,
    requestExpiredAtMillis: millis(doc.requestExpiredAt),
    expiryReason: doc.expiryReason || "",
    retryAllowed: doc.status === "expired",
...
function bookingIdentityClauses(value) {
  ...
  if (!id) return [];
  const clauses = [{ bookingId: id.toUpperCase() }, { bookingCode: id }, { publicId: id.toUpperCase() }];
